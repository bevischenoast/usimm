...
uns os_vpn_to_pfn(OS *os, uns64 vpn, uns tid, Flag *hit)
{
   ...
   // vpn = (tid<<28)+vpn; // embed tid information in high bits
    //assert(vpn>>36 == 0); //x86-64 system uses 48 bit virtual address space; 
    vpn = (((uns64)(tid))<<36)+vpn; // embed tid information in high bits
...
uns    os_get_victim_from_ipt(OS *os)
{
    ...
    random_invalid_tries=OS_NUM_RND_TRIES;
