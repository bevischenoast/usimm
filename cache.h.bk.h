...
void init_cache(MCache* c, uns sets, uns assocs, uns repl, uns block_size, int compression_enabled, int compression_mode, int max_blocks_per_line);

int isHit(MCache* c, Addr addr, Flag dirty, uns comp_size);

//MCache_Entry expanded_line_victims(MCache *c, Addr addr);
...
int mcache_access_yacc(MCache *c, Addr addr, Flag dirty, uns comp_size);  //true: hit, false: miss
...
void print_superblock_stat(MCache *c);
